...
export class ProjectRuntime {
  ...
  addAgent(kind: string, opts: { id?: string; role?: string } = {}): AgentConfig {
    ...
    // Registered is not the same as offered. The Antigravity CDP bridge still
    // builds — projects that already name it must keep opening — but this is the
    // endpoint the "add agent" rail drives, and the rail only ever lists ADES.
    // Accepting a kind that no view offers meant a withdrawn agent could be put
    // in a roster by guessing its name, and then sat there unadvertised and
    // unexplained. Refuse it here, and name what replaced it.
